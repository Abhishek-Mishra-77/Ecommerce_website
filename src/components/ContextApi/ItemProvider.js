...
const ItemProvider = (props) => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(
            `https://ecommerceapp-121ff-default-rtdb.firebaseio.com/${modifiedEmail}.json`,
            {
              method: "GET",
              headers: {
      ...
      if (items.length > 0) {
        ...
        if (response.ok) {
          ...
          if (!dataItems) {
            ...
            const uniqueId = Object.keys(data).find(
              (key) => data[key] === dataItems
            );
            const response = await fetch(
              `https://ecommerceapp-121ff-default-rtdb.firebaseio.com/${modifiedEmail}/${uniqueId}.json`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
  ...
  const contextValue = {
    ...
    onProductsCheckOutHandler: onProductsCheckOutHandler,
  };
